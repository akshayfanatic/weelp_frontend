 ...
 */
export function ComboboxMultipleAttribute({
  attributes,
  value = [],
  onChange,
}) {
  ...
  const handleSelect = (attrId, attrValue) => {
    const exists = value.some(
      (s) => s.attribute_id === attrId && s.attribute_value === attrValue
    );
    ...
    const newSelection = exists
      ...
      : [...value, { attribute_id: attrId, attribute_value: attrValue }];
              ...
              <CommandGroup key={attr.id} heading={attr.name}>
                {attr.values.split(",").map((attrValue) => {
                  ...
                  const isSelected = value.some(
                    (s) =>
                      s.attribute_id === attr.id &&
                      s.attribute_value === trimmedValue
                  );

                  return (
                    <CommandItem
                      key={`${attr.id}-${trimmedValue}`}
                      value={trimmedValue}
                      onSelect={() => handleSelect(attr.id, trimmedValue)}
                    >
                      {trimmedValue}
                      <Check
                        className={cn(
