...
const Minicart = ({ showCart, setShowCart }) => {
  ...
  return (
    <div id="portal_minicart" className={`fixed z-[30] top-0 right-0 w-full h-full max-w-[485px] min-h-screen ${showCart ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}>
      <div className={`relative transition-transform ease-in-out duration-300 max-w-[485px] bg-[#F3F5F6] h-full w-full shadow-2xl ${showCart ? "translate-x-0" : "translate-x-full"}`} style={{ willChange: "transform" }}>
        {cartItems && cartItems.length > 0 ? (
          // if data exist
          <div className="px-8 py-8 flex flex-col gap-2 h-full min-h-full tfc_scroll overflow-y-scroll group">
            <X onClick={() => setShowCart(false)} className="self-end cursor-pointer size-10 p-2 absolute top-2 z-10 left-1/2 bg-[#5a5a5a] text-white ease-in-out duration-200  -translate-y-12   group-hover:translate-y-0   rounded-full" />
