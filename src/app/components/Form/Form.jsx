...
import { MapPin, Calendar, Users, Minus, Plus, Watch } from "lucide-react";
...
import { getCitiesRegion, getCitiesRegions } from "@/lib/services/global";
...
export default function BookingForm() {
  ...
  const onSubmit = async (data) => {
    ...
    const endDate = data?.dateRange?.to
      ? data.dateRange.to.toISOString().split("T")[0]
      : "";

    const quantity =
      data?.howMany?.adults + data?.howMany?.children + data?.howMany?.infants;

    // Construct query string
    router.push(
      `/search?location=${String(
        data?.whereTo
      ).toLowerCase()}&start_date=${startDate}&end_date=${endDate}&quantity=${quantity}`
    );
  ...
  const total =
    watchedhowMany?.adults + watchedhowMany?.children + watchedhowMany?.infants;
              ...
              <div className="p-2 sm:p-4">
                ...
                >
                  ...
                  {watchedFrom?.from && watchedFrom?.to
                    ? `${new Date(watchedFrom.from).toLocaleDateString(
                        "en-US",
                        {
                          month: "short",
                          day: "2-digit",
            ...
            {showHowMany && (
              ...
              >
                <div className="bg-white w-fit p-4 px-6 rounded-lg flex flex-col gap-4 border">
                  ...
                  <button
                    type="submit"
                    className="w-full py-2 bg-secondaryDark text-white rounded-md shadow"
                  >
                    Submit
                  </button>
                </div>
