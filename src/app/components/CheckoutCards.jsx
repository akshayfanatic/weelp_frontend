...
export const CheckoutUserDetailCard = ({ userImagesrc, userName, userEmail, userPhoneno }) => {
    return (
        <div className='flex gap-4 p-4 items-center hover:bg-[#e9f3ee] cursor-pointer'>
            <img src={userImagesrc || "/assets/testimonial.png"} alt="userlogo" className='size-12 rounded-full' />
            <div className='flex flex-col'>
                ...
                <span className='text-[#4D4D4D] text-base leading-6'>{userPhoneno || "Phone: +97456314789"}</span>
            </div>
        </div>
    )
}
...
export const CheckoutItemCard = ({ productTitle, totalPassenger, totalTime }) => {
    return (
        <div className='bg-white max-w-md flex flex-col rounded-xl p-6 gap-2'>
            <h3 className='text-Blueish font-semibold text-lg'>{productTitle || "Melaka Wonderland Water Theme Park"}</h3>
            <div className='flex items-center gap-2 text-[#5A5A5A] text-sm'>
                <User size={20} /><span className='font-medium'>{totalPassenger || "2 Adults, 1 Teen"}</span>
            </div>
            <div className='flex items-center gap-2 text-[#5A5A5A] text-sm'>
                <Calendar size={20} /><span className='font-medium'>{totalTime || "3 Oct, 11:30 AM"}</span>
            </div>
        </div>
    )
}

export const CheckoutPrivateTransferCard = ({ productTitle, carName, totalTime, imageSrc }) => {
    return (
    ...
    return (
        <div className='mt-6'>
            <div className='flex gap-2'>
                ...
                <span className='flex gap-1 font-semibold'><Star className='text-xl size-5 text-[#00B67A] fill-[#00B67A]' />Trustpilot</span>
            </div>
            ...
            <div className='mt-4'>
                ...
                <p className='text-sm text-black'>Visit the Weelp Help Centre for any further <br /> questions.</p>
            </div>
            <div className='mt-4 flex flex-col gap-2'>
                <p className='flex items-center gap-2 text-xs text-black'><Phone size={16} className='fill-[#4D4D4D]' /><span>+1 (702) 648-5873</span></p>
                <p className='flex items-center gap-2 text-xs text-black'><MessageCircleMore size={16} className='text-[#4D4D4D]' /><span>Chat now</span></p>
            </div>
        </div>
    )
}