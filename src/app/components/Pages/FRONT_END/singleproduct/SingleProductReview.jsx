      ...
      <div className='flex flex-col gap-4'>
        ...
        <div className='flex justify-end sm:justify-between'>
          ...
          <div className='relative'>
            <button
              className='flex items-center gap-4 text-grayDark border text-base p-2 px-6  rounded-lg ret'
              onClick={handleSort}
            >
...

            {
              ...
              <div className='absolute z-10 right-0 mt-2'>
                ...
                <ul className=' border flex flex-col bg-white rounded-md text-sm'>
                  ...
                  <li className='cursor-pointer ease-in-out duration-200 p-4 capitalize hover:bg-[#f2f7f5] text-nowrap  text-grayDark' onClick={handleSortValue} value={5}>Sort By Rating: low to high</li>
                  <li className='cursor-pointer ease-in-out duration-200 p-4 capitalize hover:bg-[#f2f7f5] text-nowrap  text-grayDark' onClick={handleSortValue} value={2}>Sort By Rating : High to Low</li>
                </ul>
              </div>
            }
          </div>
        ...
        <div className='flex flex-col gap-4'>
          {allReviews && allReviews.map((val, index) => {
            if (index > 1) {
              return null
            }
            return (
              <ReviewCard2 key={index} userImageSrc={val?.userImageSrc || null} galleryImages={val?.galleryImages || null} userName={val?.userName || null} date={val?.date || null} title={val?.title || null} rating={val?.rating} comment={val?.comment} />
            )
          })}
        </div>
      </div>
