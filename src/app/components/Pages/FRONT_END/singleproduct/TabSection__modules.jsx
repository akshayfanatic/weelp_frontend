...
export const ProductForm = ({ productId, productData }) => {
  const gradientStyle = {
    background:
      "linear-gradient(180deg, rgba(255,255,255,1) 5%, rgba(236,255,232,1) 100%)",
  };
  return (
    <div className="p-6 sm:p-12 sm:pe-32 bg-no-repeat bg-bottom bg-cover sm:sticky sm:top-12 sm:right-0">
      <h3 className="text-Blueish font-bold text-xl sm:text-2xl capitalize">
      ...
      <div
        className="flex gap-4 flex-wrap items-start sm:items-center sm:justify-between border p-6 rounded-xl"
        style={gradientStyle}
      >
        <div className="flex flex-col gap-2">
          <h2 className="text-Blueish font-semibold capitalize text-lg">
            Questions?
          </h2>
          <p className="text-base text-black text-wrap w-4/5">
            Visit the Weelp Help Centre for any further questions.
          </p>

          <span className="sm:text-base mt-2 sm:mt-4">
            Product ID : {productId ?? 451245}
          </span>
        </div>
        <button className="p-4 font-medium text-xs sm:text-base capitalize border rounded-md h-fit w-fit text-black border-black">
          Help Center
        </button>
      </div>
    </div>
...
export const ProductFormItinerary = ({ productData }) => {
  ...
  return (
    <div className="p-6 sm:p-12 sm:pe-32 bg-no-repeat bg-bottom bg-cover sm:sticky sm:top-12 sm:right-0">
      <h3 className="text-Blueish font-bold text-xl sm:text-2xl capitalize">
        From $ {productData?.base_pricing?.variations[0]?.regular_price}
      </h3>
      <BreakSection marginTop={"mt-9"} />
