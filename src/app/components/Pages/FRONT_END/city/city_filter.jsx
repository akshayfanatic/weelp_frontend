...
export const CityFilter = () => {
  ...
  const debouncedFetchProducts = useCallback(
    _.debounce(() => {
      ...
      axios
        .get(
          `${process.env.NEXT_PUBLIC_API_BASE_URLL}api/${city}/all-items${query}`
        )
        .then((response) => {
  ...
  const categoryList = useMemo(
    () =>
      categories.map((category, index) => (
        <label
          key={index}
          className="flex items-center space-x-2 cursor-pointer text-md font-medium text-grayDark"
        >
          <input
            type="checkbox"
            value={category?.name}
  ...
  return (
    <div className="flex flex-col sm:flex-row sm:gap-4 lg:gap-8 p-4">
      ...
      <div className="w-full lg:flex-[4]">
        ...
        <div
          className={`flex flex-col sm:flex-row sm:flex-wrap gap-y-4 gap-4 py-4 sm:py-12 ${
