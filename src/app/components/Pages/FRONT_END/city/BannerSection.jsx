...
const BannerSection = () => {
    ...
    const fetchcityDetails = async () => {
      try {
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_API_BASE_URLL}api/city/${city}/`
        );
  ...
  if (initialize) {
    ...
    return (
      <section
        className="flex lg:h-[60vh] py-6 relative page_city_banner"
      ...
      >
        <div className="flex flex-col lg:flex-row container mx-auto gap-4 p-6">
          <div className="relative flex-1 w-full lg:w-1/3 py-4 ">
            <h2 className="text-2xl font-medium text-[#143042] mb-4 capitalize">
              Things to do In
            </h2>
            <h1 className="font-semibold text-5xl text-[#143042]  mb-4 capitalize">
              {name}
            </h1>
            <p className="font-medium text-lg text-grayDark text-wrap">
              {description}
            </p>
        ...
        <Icons.Vector2
          className={
            "hidden lg:block absolute bottom-0 left-0 -translate-x-14 scale-125 rotate-45"
          }
        />
        <Icons.Vector2
          className={
            "hidden lg:block absolute bottom-16 left-4 rotate-45 scale-[.2]"
          }
        />
      </section>
    );
