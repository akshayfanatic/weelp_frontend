...
export const ShopAllProduct = () => {
  ...
  const fetchProducts = useCallback(
    _.debounce(() => {
      ...
      if (selectedLocation) {
        query += `&${selectedLocation.type}=${String(
          encodeURIComponent(selectedLocation.name)
        ).toLowerCase()}`;
      }
        ...
        <div className="w-full flex flex-col gap-4 p-4 md:px-8 ">
          ...
          <div className="w-full flex gap-4 flex-col sm:flex-row">
            ...
            <div className="w-full h-fit sm:max-w-xs p-4 bg-white shadow-md rounded-lg">
              {/* Category Filters */}
              <h2 className="text-lg font-medium text-[#143042] my-4">
                Category
              </h2>
                ...
                {useMemo(
                  () =>
                    categories.map((category) => (
                      <label
                        key={category.id}
                        className="flex items-center space-x-2 cursor-pointer text-grayDark"
                      >
                        <input
                          type="checkbox"
                          value={category.name}
                    ...
                    products.map((product, index) => (
                      <GlobalCard
                        ...
                        productPrice={
                          ...
                          product?.price?.variations[0]?.regular_price
                        }
                      />
                    ))
                  ) : (
