...
export const WhatAboutCity = ({  location_details }) => {
  ...
  const metaData = Object.entries(location_details ?? {}).length > 2
  ? Object.entries(location_details).slice(2, 9)
  : [];
    ...
    <div className="p-6 sm:py-8 sm:px-16 xl:px-8 2xl:px-16">
      {metaData.length > 0 ? (
        <>
          ...
          <h2 className="font-semibold text-3xl text-[#143042] capitalize">
            {city}
          </h2>
            ...
            {metaData.map((val, index) => {
              if (val[0] && val[1]) {
                return (
                  <li
                    key={index}
                    className="capitalize flex flex-col flex-wrap items-start p-2 sm:py-9 sm:px-13 odd:border-b odd:border-r even:border-b"
                  >
                    <h3 className="font-semibold px-8 text-sm sm:text-xl">
                      {stringSignRemover(val?.[0])} 
                    </h3>
                    <span className="capitalize text-xs sm:text-sm font-medium px-8 text-grayDark">
                      {val[1]}
                    </span>
                  </li>
                );
